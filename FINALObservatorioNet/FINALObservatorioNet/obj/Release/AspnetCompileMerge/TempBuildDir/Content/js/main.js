...
$(document).ready(function () {
    $.getJSON('data/agrupacion.json', callbackmydata);
    function callbackmydata(data) {
        // agrupacion únicas
        var texto = 'Abonos_orgánicos';
        var js = data;
        var agrupa = Object.keys(js);
        var categoria = "";
        console.log(agrupa[0]);
        $('.agrupacion').select2(
            {
                theme: "bootstrap",
                placeholder: 'Todos',
        ...
        $('.agrupacion').on('change', function () {
            ...
            var agrupado = $(".agrupacion option:selected").text()
            if (agrupado == "") {
                agrupado = "Todos";
            }
            var cat2 = js[agrupado];
            texto = js[agrupado][0].replace(/\s/g, "_");
            categoria = js[agrupado][0].replace(/\s/g, "_");
            $('.categorias').select2(
                {
                    theme: "bootstrap",
                    placeholder: js[agrupado][0],
                    allowClear: false,
                    data: cat2,
                    templateResult: function (data) {
                        return data.text;
        ...
        var cat = $.map(js, function (obj) {
            obj.text = obj.text
            return obj;
        });
        var cat = cat.filter(onlyUnique);
        $('.categorias').select2(
            {
                theme: "bootstrap",
                placeholder: "Abonos orgánicos",
                allowClear: false,
                data: cat,
                templateResult: function (data) {
                    return data.text;
...
function grafo(texto) {
    ...
    texto = texto.replace("/", "");
    var urlLUIS = "";
    urlLUIS = "data/categorias/" + texto + ".json";
    ...
    var graph = d3.json(urlLUIS, function (error, graph) {
        ...
        var data = '';
        if (texto == '') {
            var data = graph.Abonos_orgánicos[0]
        }
        else {
        ...
        var circle = node.append("circle")
            .attr("r", function (d) { if (monto_max == monto_min) { return Math.log(d.monto) + 3 } else { return ((d.monto - monto_min) / (monto_max - monto_min) * 40) + 3 }; })
        ...
        legend.append("rect")
            .attr("x", window.innerWidth - window.innerWidth / 10)
        ...
        legend.append("text")
            .attr("x", window.innerWidth - window.innerWidth / 9.5)
            .style("font-family", "Roboto")
            .style("font-weight", "100")
            .style("fill", "black")
            .attr("y", window.innerHeight - window.innerHeight / 3.1)
            .attr("dy", ".35em")
        ...
        g.append("g")
            ...
            .style("fill", "grey")
            .attr("transform", "translate(" + ((10)) + ", " + (window.innerHeight - window.innerHeight / 3) + ")")
        ...
        g.append("text")
            ...
            .style("font-weight", "200")
            .style("fill", "grey")
            .attr("transform", "translate(" + ((10)) + "," + (window.innerHeight - window.innerHeight / 2.83) + ")");
